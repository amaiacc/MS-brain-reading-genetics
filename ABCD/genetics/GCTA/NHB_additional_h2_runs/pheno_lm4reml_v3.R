...
setwd(pheno_dir)
base_dir=paste(geno_dir,"/GCTA/pheno_v3/",sep="")
if (!dir.exists(base_dir)){dir.create(base_dir)}

# read data
info<-read.table(paste(pgeno_dir,"ABCD_release3.0_.batch_info.txt",sep=""),header=TRUE)
dat <- readRDS(paste0(pheno_v,"_baseline_sMRI.Rds"))
ids<-read.table(paste(geno_dir,"/GCTA/grm/",root,".grm.id",sep=""))
pcs<-read.table(paste(geno_dir,"preImputation_QC/intermediate_files/sampleQC/PopStr/","ABCD_release_3.0_QCed_EUR_6sd.pop_strat_outliers.pca.evec",sep=""),stringsAsFactors = FALSE) # need to make this dependent on root
...
d<-merge(d,ids,by.x="subjectid",by.y="V2")
