...
if (file.exists(paste(out_dir,"/tables/baseline_",m,"intAge_",atlas,"_","summary_t_table.csv",sep=""))==FALSE) # only run if output file does not exist 
{
  for (iv in rois2run){
    # if(iv!=global_var){
      # define test model
      ...
      anovat<-anova(m_int_t)[int_term,] %>% data.frame() %>% mutate(model=int_term)
      # LRT  (REML)
  ...
  # save summary tables
  write.csv(anova_table,file=paste(out_dir,"/tables/baseline_",m,"intAge_",atlas,"_","anova_table.csv",sep=""),row.names = FALSE)
  write.csv(sum_table,file=paste(out_dir,"/tables/baseline_",m,"intAge_",atlas,"_","summary_t_table.csv",sep=""),row.names = FALSE)
  write.csv(lrt_table,file=paste(out_dir,"/tables/baseline_",m,"intAge_",atlas,"_","LRT_table.csv",sep=""),row.names = FALSE)
  write.csv(effect_table,file=paste(out_dir,"/tables/baseline_",m,"intAge_",atlas,"_","effect_r2m_table.csv",sep=""),row.names = FALSE)
  # clean
