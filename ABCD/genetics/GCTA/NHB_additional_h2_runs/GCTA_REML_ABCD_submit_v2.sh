...
for phenotype in ${phenos2check}
 do
 if [ ! -f ${working_dir}/pheno_v2/residuals_${phenotype}.txt ]; then 
  Rscript ${scripts_dir}pheno_lm4reml_v2.R ${root} ${phenotype} ${pheno_v}
 fi
done
...
for phenotype in ${phenos2check}
 ...
 then
  qsub ${scripts_dir}GCTA_REML_ABCD_template.sh ${root} ${phenotype}_residuals ${working_dir}/pheno_v2/residuals_${phenotype}.txt
 fi
done
...
for phenotype in ${rois2check}
 ...
 then
  f=$(ls ${working_dir}/pheno_v2/residuals_adjGlobal*.*_${phenotype}.txt )
...
# organize output
#--------------------------------------------
mv ${working_dir}/reml ${working_dir}/reml_v2
# extract summary tables for h2 and rho
cd ${working_dir}/reml_v2/
#mv *log logs/
