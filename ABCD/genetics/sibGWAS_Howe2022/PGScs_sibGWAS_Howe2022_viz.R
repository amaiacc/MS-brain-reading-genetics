...
PGS_dir=paste(dir,project,"/data/working_data/genotyping/PRScs/",base_project,"/",sep="")
...
ys_PGS_plot_c<-ggplot(data=subset(pgs_summary,trait==base_p&subset==s) %>%  
                      ...
                      aes(x=threshold,y=adjR2*100,fill=-log10(P))) +
  ...
  labs(y=expression("% variance " (Delta~R^2)),x="PGS-CS",
...
reading_best<-reading_best %>% filter(adjR2==max(reading_best$adjR2))
...
ys_PGS_plot_c2 <- ys_PGS_plot_c + geom_rect(aes(
  ...
  ymin =0, ymax = max(reading_best$adjR2)*100),
...
#---------------------------------------------------
ggsave(PGS_all_plot_c,file=paste0(out_dir,"PGScs_effects_",base_project,"_","2ABCD_",s,"_behav.pdf"),width=11,height=3)
